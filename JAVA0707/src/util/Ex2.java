...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 * 	  따라서, reset() 메서드를 통해 인덱스를 초기화한 후 다른 작업을 수행하는 것이 좋다.
		 * 	  또는, find() 메서드 파라미터로 시작인덱스 지정이 가능하므로
		 * 	  find(0) 형태로 0번
		 * ----------------------------------------------------------------------------
		 * 5) replaceFirst()	: 문자열 중 정규표현식과 처음 일치하는 문자열을 치환
		 * 6) replaceAll()		: 문자열 중 정규표현식과 일치하는 문자열을 치환
		...
		// find() 메서드 파라미터로 검색할 인덱스 번호를 첫 인덱스 번호인 0 지정해도 됨
		System.out.println("문자열이 정규표현식을 포함하는가? " + matcher.find(0));
		
		matcher.reset();
		int cnt = 0;
		while(matcher.find())	cnt++;
		System.out.println("문자열에 Java가 포함된 개수 : " + cnt);
