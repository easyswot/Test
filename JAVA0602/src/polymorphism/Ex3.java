...
public class Ex3 {
	...
	public static void main(String[] args) {
		...
		emp = man;	// 업캐스팅 자동 형변환 가능(묵시적 형변환) - 형변환 연산자 생략 가능
		// 업캐스팅 후에는 참조영역에 대한 축소가 발생
		// 단, 현재 manager 클래스에는 별도의 추가 메서드가 없으므로 기능 동일함
		System.out.println("emp.getEmployee : " + emp.getEmployee());
		// => 코드상(컴파일시점)으로는 Employee 클래스의 getEmployee() 메서드가 호출될 것처럼 보이지만
		//	  실제 emp변수에 저장된 인스턴스(객체) Manager 클래스의 인스턴스 이므로
		//	  실행 시점에는 매니저 클래스에 오버라이딩된 getemployee() 메서드가 실행됨
		
		System.out.println("----------------------------------------");
		
		// subclass engineer instance 생성
		// => 강감찬, 2000, 자바개발
		Engineer eng = new Engineer("강감찬", 2000, "자바개발");
		System.out.println("eng.getEmployee() : " + eng.getEmployee());
		
		// engineer -> employee up
		Employee emp2 = eng;
		System.out.println("emp.getEmployee() : " + emp2.getEmployee());
		
		// manager emp2 up
		emp2 = new Manager("가나다", 5000, "개발팀");
		...
		// -------------------------------------------------------------------
		Manager[] arrMan = { new Manager("hgd", 100, "make")
						   , new Manager("af", 150, "af")
						   , new Manager("afdfs", 200, "sff")};
		for(Manager manager : arrMan) {
			System.out.println(manager.getEmployee());
		}
		
		Engineer[] arrEng = { new Engineer("gd", 100, "ke")
							, new Engineer("hf", 130, "alh")
							, new Engineer("dgfs", 200, "adsf")};
		
		for(Engineer engineer : arrEng) {
			System.out.println(engineer.getEmployee());
		}
		
		Employee[] arrEmp = { new Manager("hgd", 100, "make")
							, new Manager("af", 150, "af")
							, new Manager("afdfs", 200, "sff")
							, new Engineer("gd", 100, "ke")
							, new Engineer("hf", 130, "alh")
							, new Engineer("dgfs", 200, "adsf")};
		...
		// 오버라이딩(동적바인딩, 메서드재정의)+업캐스팅(타입변환)=> 다형성을 구현할 수 있다.
