...
public class Ex3 {
	...
	public static void main(String[] args) {
		/*
		 * static 메서드(=정적 메서드)
		 * - 메서드 선언 시 리턴 타입 앞에 static 키워드를 붙여서 정의한 메서드
		 * - static 멤버변수와 마찬가지로 클래스로딩 시 함께 메모리에 로딩되므로
		 *   인스턴스 생성과 관계없이 클래스명만으로 호출 가능한 메서드
		 *   (클래스명, 메서드명() 형태로 호출)
		 *   
		 *   <주의사항>
		 * 1. static 메서드 내에서는 인스턴스 멤버변수에 접근 불가
		 * 		=> static 메서드가 로딩되는 시점에서는 인스턴스가 생성되닉 전이므로
		 * 		   인스턴스 멤버변수도 로딩되지 않은 상태
		 * 		   따라서, static 메서드 호출 시점에 인스턴스 멤버변수가 존재하지 않을 수 있다
		 * 2. static 메서드 내에서 this, super 사용 불가
		 * 		=> static 메서드가 로딩되는 시점은 인스턴스 생성 전이므로
		 * 			인스턴스 주소가 저장되는 레퍼런스 this, super는 존재하지 않음(생성 전)
		 * 3. static메서드 내에서 일반 메서드 호출 불가
		 * 		=> 사유 동일
		 * 		=> static 메서드만 호출 가능
		 * 결론 static 메서드 내에서 static 멤버만 접근 가능
		 * 
		 * */
...
//		this.staticVar = staticVar;	// 오류! static 메서드 내에서 this 사용 불가!
		...
		//    따라서, static 메서드 내에서는 this사용이 불가능하다!
