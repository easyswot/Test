...
class StaticMember {
	static int a = 10;	// 클래스(class) 멤버변수(클래스 로딩 시 단 한번 메모리 할당 받음)
	int b = 20;			// 인스턴스 멤버변수(인스턴스 생성할 때마다 베모리 할당 받음)
...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * < static 키워드 >
		 * - 정적(고정됨) 이라는 의미를 갖는 키워드
		 * - 클래스, 메서드, 변수의 지정자로 사용 가능
		 * - 멤버 베서드 또는 변수에 static 키워드를 사용할 경우
		 ...
		 *   
		 * < Static 메서드 >
		 * - static 멤버변수와 마찬가지로 클래스로딩 시 함께 메모리에 로딩되므로 클래스명만으로 호출 가능한 메서드
		 * 	 (클래스명, 메서드명())
		 * 
		 * 
		 * */
...
//		System.out.println("StaticMember.b = " + StaticMember.b);
		...
		// 주로 클래스명 멤버변수명 형태로 접근하는 것이 올바른 접근방법
		StaticMember.a = 99;
		System.out.println("s1.a = " + s1.a + ", s2.a = " + s2.a);
		System.out.println("s1.b = " + s1.b + ", s2.b = " + s2.b);
		//=> s1 인스턴스의 멤버변수 a의 값만 변경하더라도 static 변수이므로 모든 인스턴스가 해당 변수값을 공유하게 되어 s2 인스턴스의 멤버변수 a도 99가 됨
