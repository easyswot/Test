...
public class Ex3 {
	...
	public static void main(String[] args) {
		 ...
		 * 		1) 멀티스레딩 코드가 포함될 서브클래스에서 Runnable 인터페이스를 구현(implements)
		 * 		2) 추상메서드인 run() 메서드를 구현 (오버라이딩)하여 멀티스레딩 코드 기술
		 * 		3) 멀티스레딩 클래스 인스턴스 생성
		...
		YourThread yt2 = new YourThread("B작업", 1000000);
//		YourThread yt3 = new YourThread("C작업", 1000000);
		Runnable yt3 = new YourThread("C작업", 1000000);		// YourThread -> Runnable 업캐스팅
		
		// Runnable 구현체 내부에는 start() 메서드가 존재하지 않는다
//		yt3.start();
		
		// Thread 클래스의 인스턴스 생성 시 생성자 파라미터로 Runnable 구현체 객체 전달 후
		// Thread 객체를 통해 start() 메서드를 호출하여 간접적으로 멀티스레딩 수행
...
//		t3.start();		
		...
		new Thread(new YourThread("B작업", 1000000)).start();
