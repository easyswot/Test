...
class C extends B implements A, AA, AAA {}
//class C implements A, AA, AAA extends B {}	// extends 즉, 상속이 먼저 선행되어야 함
...
// 다중상속시 부모의 메서드가 동일할 경우 메서드 접근의 모호성이 발생할 수 있으므로
// 자바에서는 다중상속을 허용하지 않는다.
//class 고래상어클래스 extends 고래클래스, 상어클래스 {
//	public void 부모의번식() {
//		번식();	// 이 번식 메서드는 고래와 상어 중 누구의 번식 메서드인가
//		// 따랏, 자바에서는 모호한 상황을 애초에 차단하기 위해 다중상속을 허용하지 않음
//	}
//}
// =======================================================================
...
class 고래상어 implements 고래인터페이스, 상어인터페이스 {
	...
	public void 번식() {
		...
		// => 모든 구현은 서브클래스에게 맡기고, 인터페이스는 메서드 형식(표현)만 규정하는 역할
