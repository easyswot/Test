...
List boardList=boardDAO.getBoardList2(keyWord, search, startRow, pageSize);
%>
<article>
<h1>Notice</h1>
<table id="notice">
...
if(startPage>pageBlock) {
	%>
	<a href="fsearchBbs.jsp?pageNum=<%=startPage-pageBlock%>&keyWord=<%=keyWord%>&searchText=<%=search%>">Prev</a>
	<%
}
for(int i=startPage;i<=endPage;i++){
	if(currentPage==i){
		%>
		<a href="fsearchBbs.jsp?pageNum=<%=i%>&keyWord=<%=keyWord%>&searchText=<%=search%>" 
			style="color: tomato; font-weight: bold; text-decoration: underline;"><%=i%></a>
		<%
	} else {
		%>
		<a href="fsearchBbs.jsp?pageNum=<%=i%>&keyWord=<%=keyWord%>&searchText=<%=search%>"><%=i%></a>
		<%
	}
}
...
if(endPage < pageCount){
	%>
	<a href="fsearchBbs.jsp?pageNum=<%=startPage+pageBlock%>&keyWord=<%=keyWord%>&searchText=<%=search%>">Next</a>
	<%
}
%>
