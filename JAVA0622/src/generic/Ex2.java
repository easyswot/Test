...
//		nic.data = "홍길동";	// 문자열 저장 불가
		// => 여러 데이터타입을 데이터를 모두 저장하려면 최소한 Object 타입 변수 선언해야 함
...
//		gc.setData("홍길동");		// 컴파일에러 발생 Integer 타입만 전달 가능
		...
		// => Object 타입 미지정과 지정의 기증상의 차이는 없으나
		//	  제네릭 타입 자체를 생략할 경우 경고메시지 표시되므로 제네릭 사용 추천
		GenericClass gc5 = new GenericClass();	// 경고 메시지 표시됨
...
class GenericClass<T> {
	...
	// => 차수, 객체 생성 시점에서 제네릭 타입에 대한 실제 데이터타입을 명시할 경우
