...
public class Ex3 {
	...
	public static void main(String[] args) {
		...
		// => 문자열을 상수 풀(Constant Poll)에서 검색하여 중복되는 문자열이 없으면 저장하고
		...
		// => 따라서, 두 문자열 저장하는 변수 s1, s2가 가리키는 주소값이 같다
		...
		// Object 클래스로부터 상속받아 오버라이딩 됨
		// String 클래스의 equals() 메서드 사용하여 비교하는 방법 = 실제 데이터(문자열) 비교
		if(s1.equals(s2)) {
			System.out.println("s1 과 s2 는 문자열이 같다.");
		} else {
			System.out.println("s1 과 s2 는 문자열이 다르다.");
		}
		
		// 객체 생성을 통한 문자열을 생성하는 방법
		// => 인스턴스 생성을 위해 new연산자 사용 시 무조건 Heap 공간에 새로운 메모리 할당
