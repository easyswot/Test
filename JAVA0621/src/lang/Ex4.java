...
public class Ex4 {
	...
	public static void main(String[] args) {
		...
		// 1) substring(int beginIdex) : 시작 인덱스부터 모든 문자열 추출
		System.out.println("s1.substring(5) : " + s1.substring(5));	// 5번 인덱스 부터 끝까지 추출
		
		// 2) substring(int beginIndex, int endIndex) : 시작인덱스부터 끝인덱스-1 까지 추출
		System.out.println("s1.substring(5, 12) : " + s1.substring(5, 12));	// 5~11번까지 추출
		
		// concat() : 문자열 결합(결합연산자(+) 보다 연산 속도가 빠르다)
		System.out.println("s1.concat(!) : " + s1.concat("!"));	// 문자열 s1뒤에 문자열 "!" 결합
		
		// indexOf() : 문자열의 앞쪽 (첫 인덱스)부터 찾고자 하는 문자 또는 문자열이 인덱스 리턴
		// => 탐색할 문자 또는 문자열이 없을 경우 -1 리턴
		System.out.println(s1.indexOf('a'));	// 앞에서부터 문자 'a'를 찾아 인덱스(1) 리턴
		System.out.println(s1.indexOf('x'));	// 'x'는 존재하지 않으므로 -1 리턴
		System.out.println(s1.indexOf("a"));	// 앞에서부터 문자열 "a"을 찾아 인덱스(1) 리턴
		System.out.println(s1.indexOf("Program"));
		
		// lastIndexOf() : 문자열 뒷쪽(끝인덱스) 부터 찾고자 하는 문자 또는 문자열의 인덱스 리턴
		// => 탐색을 거꾸로 할 뿐, 인덱스 번호 자체가 변하지 않는다
		...
		// => 문자열 입력 시 의도치 않은 불필요한 공백이 삽입되는 것을 방치할 때 사용
		...
		// String s3 = "자바/JSP/안드로이드";
		String[] subjects = s3.split("/");	// / 문자열을 기준으로 문자열을 분리
		...
		// ---------------------------------------------------------------------------------
		// String.format() 메서드 : 특정 문자열을 형식 지정문자와 결합하여 형식을 부여
		// => System.out.printf() 메서드 역할과 동일하나 출력을 위한 것이 아니라
		//	  문자열을 생성할 때 형식을 지정하기 위함
		String name = "홍길동";
		int age = 20;
		double height = 180.5;
		// => 위의 세가지 데이터를 다음과 같이 문자열로 결합하여 변수 total 저장
