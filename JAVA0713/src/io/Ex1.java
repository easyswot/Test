...
public class Ex1 {
	...
	public static void main(String[] args) {
		 ...
		 * 				(입력노드: 키보드, 마우스, 파일, 네트워크, 데이터베이스 등)
		 * 				(출력노드: 모니터, 스피커, 파일, 네트워크, 데이터베이스 등)
		 ...
		 * 		3) System.err : 모니터에서 에러 정보 출력 (잘 사용하지 않음)
		 * 
		 * 
		 * */
...
//		}
		...
		try(InputStream is = System.in){
			...
			// 반복문을 사용하여 더이상 읽어올 데이터가 없을때(-1) 까지 입력을 처리
			while(n != -1) {
				System.out.println("입력 데이터: " + n + ", 문자로 변환: " + (char)n);
				n = is.read();
			}
		} catch (IOException e) {
			e.printStackTrace();
