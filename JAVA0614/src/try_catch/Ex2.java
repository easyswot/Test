...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 * - 예외처리를 위해 try ~ catch 문을 사용하여 처리 작업 수행
		 * 	 => try 블록 내에서 계외 발생 가능성이 있는 코드들을 감시하고
		 ...
		 * 		  => 예외가 발생할 것으로 예상되는 코드를 찾아서 예외처리를 수행해야 함
		...
		try {
			...
			// 현재 예외가 발생한 코드의 아래쪽 나머지 코드들(try 블록 내의 코드들)은
			// 실행되지 못하고, catch 블록으로 가서 예외 객체에 해당되는 클래스를 찾아
		...
		// catch 블록 코드 실행 후 try ~ catch문 밖으로 나와 나머지 코드를 정상적으로 실행함
		// => 즉, 예외가 발생하더라도 프로그램이 정상적으로 종료되도록 처리 됨
		...
		System.out.println("program 종료");	//NullPointerException 예외 발생
