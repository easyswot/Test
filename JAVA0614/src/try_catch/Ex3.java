...
public class Ex3 {
	...
	public static void main(String[] args) {
		 ...
		 * - 복수개의 catch 블록은 첫번째 catch 블록 부터 차례대로 탐색함
		 * 	 => 만약, 끝까지 탐색 했음에도 일치하는 catch 블록이 없으면 실행 시 예외 발생함
		 * - 만약, 복수개의 catch 블록 지정 시 하위 타입부터 상위 타입순으로 나열해야함
		 * 	 => 상위 타입이 먼저 기술되면 하위 타입 예외는 실행될 수 없다
		 *   (ex. ArithmeticException 보다 Exception클래스가 위에 있을 수 없다)
		 * - 만약, 하나의 catch 블록으로 복수개의 예외를 처리하려면
		 * 	 1) catch 블록의 클래스를 복수개의 예외 클래스와 상위 타입으로 지정하면 된다
		 * 	    (ex. 모든 예외를 처리하기 위해서는 Exception 클래스를 지정)
		 * 	 2) catch블록의 클래스에 | 기호를 사용하여 복수개의 클래스를 기술해도 된다.
		 * 		=> 유사한 성격을 갖는 예외를 하나의 catch 블록에서 묶어서 처리 가능
		 * 		(ex. FileNotFoundException 과 ClassNotFoundException은 유사한 기능이므로
		 ...
		 *  
		 *  } catch(Exception e) {
		 *  	// 위의 모든 예외가 일치하지 않을 경우 나머지 예외 모드 처리할 코드
		...
		try {
			// 첫 번째 예외 : ArithmeticException
			int num1 = 3, num2 = 0;
			System.out.println(num1/num2);
			
			// 두 번째 예외 : NullPointerException
			String str = null;
			System.out.println(str.length());
			
			// 세 번째 예외 : ArrayIndexOutOfBoundsException
			int[] arr = {1};
			...
			// 위의 세가지 예외에 해당되니 않는 예외를 모두 Exception 타입으로 catch함
			// => if문에서 else문 역할과 동일함
			System.out.println("");
		}
		...
		try {
			...
			System.out.println("Exception 클래스가 예외 처리");
		}
