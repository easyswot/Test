...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 * 인터페이스 필요성 (장점)
		 * 1. 구현의 강제성 부여 (= 통일성)
		 * 		- 인터페이스를 상속받은 서브클래스는 반드시 추상메서드를 구현해야 한다. (메서드 이름 통일)
		 * 
		 * 2. 모듈 교체의 용이 (= 무기교체, 전자기기 충전 c타입 usb)
		 * 		- 부모 타입인 인터페이스 타입을 변수로 사용하여 각 서브클래스를 다룰 경우
		 * 		  특정 서브클래스가 또 추가되더라도 공통 코드의 변경이 필요없이
		 * 		  새로운 클래스만 저으이하게 되면 언제든지 인스턴스 교체를 통해 서브클래스를 다룰 수 있음
		 ...
		 * 	  (Bird, SuperMan, Airplane -> Flyer)
		 * 
		 * 4. 모듈 간 독립적 프로그래밍으로 인한 개발 기간 단축
		 * 
		 * */
		...
		pc.setPrinter(new InkjepPrinter());
		pc.print("Test2.java");
...
class InkjepPrinter implements Printer {
...
class PrintClient {	// 사용자
	// PrintClient 클래스에서 Printer 인터페이스를 전달 받아 프린터를 컨트롤하는 경우
