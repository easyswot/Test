...
public class Ex1 {
	...
	public static void main(String[] args) {
		...
		// 클래스가 클래스를 상속 extends (다중x)
		// 클래스가 인터페이스를 상속(구현) implements (다중o)
		// 인터페이스가 인터페이스를 상속(확장) extends (다중o)
		// 인터페이스가 클래스를 상속 -> 이런 경우 자체가 없음
...
// 인터페이스 끼리 상속을 받을 경우 extends 키워드를 사용해야 한다
// => 인터페이스가 가질 수 있는 메서드는 모두 추상메서드이므로
...
// 	  서브 인터페이스도 추상메서드 형태로 보관하기 때문이다.
interface ITotal extends IHello, IGoodbye {
//	public void sayHello(String name) {} // err 추상메서드 구현이 불가능
	public void greeting(String name);
}

// 2개의 인터페이스를 상속받은 ITotal 인터페이스를
// 서브클래스에서 상속받아 구현하면 모든 인터페이스의 내용을 구현하게 된다
// 만약, 하나라도 추상메서드를 구현하지 않으려면 추상클래스로 선언해야 된다
...
class ISay2SubClass extends ISay2 {
	...
	// 추상클래스인 ISay2 클래스를 상속받더라도 미구현 된 greeting() 메서드만 구현하면 된다
...
class NormalClass extends AbstractClass2{
	...
	// 추상메서드 2개를 갖은AbstractClass2를 상속받으면 2개의 추상메서드 모두 오버라이딩 필수
