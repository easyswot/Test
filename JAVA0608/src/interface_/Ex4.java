...
public class Ex4 {
	...
	public static void main(String[] args) {
		 ...
		 * 	 상대방의 작업 진행 여부와 관계업ㅎ이 임의의 데이터로 테스트가 가능하다
		...
		DeveloperLogin d1 = new DeveloperLogin();
		String result = d1.login("admin", "1234");
...
// 개발자(백엔드)와 디자이너(프론트엔드)가 수행해야하는 작업들의 공통 부분을 추상메서드로 정의하여
// 기본적인 틀을 제시하면 개발자와 디자이너는 각각 별도로 독립된 작읍 수행이 가능하다
interface LoginInterface {
	// 디자이너는 입력받은 id, password를 문자열로 전달하면 어떤 문자열이 리턴되겠구나
	// 개발자는 id, password를 전달받아 로그인 처리 후 결과를 문자열로 리턴 해주면 되겠구나
	public String login(String id, String password);
}

// 개발자가 수행해야하는 로그인 처리 작업
// => 공통 인터페이스를 상속받아 추상메서드를 구현하여 로그인작업 수행 가능
...
class DesinerLogin implements LoginInterface {
	...
	public String login(String id, String password) {
		...
		// 개발자와 별가로 독립적인 작업을 위해 공통 메서드를 임시로 구현하여
		// 개발자 입장에서 처리할 코드는 생략하고 (DB select, if - else로 판별하는 작업)
