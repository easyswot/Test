...
public class Ex1 {
	...
	public static void main(String[] args) {
		 ...
		 * 		(따라서, 메서드 선언부 마지막 바디{} 대신 세미콜론으로 끝냄)
...
//		new MiddleClass();
		...
		// 추상클래스는 인스턴스 생성은 불가능하지만, 다형성 활용을 위해 변수로 사용은 가능
		// => 즉, 변수의 클래스 타입으로 사용할 수 있다.
		AbstractClass ac = new SubClass2();	// SubClass2 -> AbstractClass2로 업캐스팅
		// 업캐스팅시 참조영역이 축소되지만, 추상메서드는 반드시 구현되어 존재하므로
		// 슈퍼클래스 타입으로 상속되어 구현된 추상메서드 호출이 가능
		ac.abstractMethod();
...
abstract class MiddleClass extends AbstractClass {
	// 추상메서드를 포함하는 추상클래스를 상속받은 경우 오버라이딩이 필수지만
