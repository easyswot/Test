...
public class Ex1 {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 1. BigInteger 클래스			금융권에서 많이 쓰임
		 * - 정수 최대 크기(long 타입이라고 가정)는 약 922경인데
		 ...
		 * 	 => xxx은 기본데이터타입 이름
		 * 	 => 각 기본데이터타입 크기에 따라 "오버플로우"가 발생하여
		 * 		올바르지 못한 데이터가 저장될 수 있다.
		 * 
		 * 2. BigDecimal 클래스			생산관리
		...
		Integer i = 10;	// Wrapper 클래스타입은 오토박싱/오토언박싱에 의해 자동변환됨
		
//		BigInteger bi = 10;	// BigInteger 클래스타입은 자동변환 지원 안됨
		// => 반드시 생성자를 통해 "문자열" 형태로 정수를 전달해야 한다.
		
		BigInteger bi = new BigInteger("999999999999999999999999999");	// 28개
		// long 타입보다 큰 정수도 문자열 형태로 전달이 가능하며 접미사 불필요
		
		// toString() 메서드가 오버라이딩 되어 있으므로 출력이 간편함
		System.out.println("저장된 숫자: " + bi);
		
		// BigInteger --> 기본데이터타입으로 변환 : xxxValue() 메서드
		int num = bi.intValue();	// BigInteger -> int로 변환(오버플로우 발생 가능성 있음)
//		System.out.println(num);
		
		BigInteger bi2 = new BigInteger("55555555555555555555555");	// 20개
//		BigInteger bi2 = new BigInteger("-55555555555555555555555");	// 20개
		System.out.println("저장된 숫자: " + bi2);
		
//		System.out.println(b1 + b2);	// 일반 산술연산자 사용이 불가능
...
//		BigDecimal bc2 = new BigDecimal(1.1);
		...
		double d3 = 1.1;
//		BigDecimal bc3 = new BigDecimal(d3);	// 기본 데이터타입 그대로 전달하면
//		System.out.println(bc3);				// 문제 발생할 수 있음
		
		// double -> 문자열로 변환하여 BigDecimal 객체 생성하는 방법
//		BigDecimal bc3 = new BigDecimal(d3 + "");	// 널스트링과 결합 문자열로 변환하는 방법 숫자와 문자열 합하면 됨
