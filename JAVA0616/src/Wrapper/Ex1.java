...
public class Ex1 {
	...
	public static void main(String[] args) {
		/*
		 * Wrapper 클래스
		 * - 자바의 기본 데이터 타입 8가지와 1:1로 대응하는 8개의 클래스의 모듬(java.lang 패키지)
		 * - 대부분 기본데이터타입 이름의 첫글자를 대문자로 사용하는 클래스명을 가지며,
		 ...
		 * 	 => 좀 더 효율적인 크로그래밍이 가능하도록 도와주는 클래스들의 모음
		 * - 기본 데이터타입 데이터(Stack 영역)를 Wrapper 클래스 객체로 변환하여 데이터 관리 가능
		 * 	 => Wrapper 클래스의 객체 주소를 Wrapper클래스 타입 참조 변수로 관리 가능(클래스 역할 동일)
		...
		System.out.println("double 타입 메모리 크기 : " + Double.SIZE + "bit");
		System.out.println("double 타입 메모리 크기 : " + Double.BYTES + "byte");
