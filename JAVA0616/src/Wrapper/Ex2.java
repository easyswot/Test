...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 * => 따라서, 기본 데이터타입 변수와 참조데이터타입 변수는 서로 호환 불가능 했음
		 * 그러나, Java(JDK) 1.5 부터 오토 박싱, 오토 언박싱 기능이 제공되면서
		 * 두 데이터타입 간에 자동으로 변환이 되도록 지원됨
		 * 
		 * 1. 오토 박싱 (Auto Boxing)
		 * - 기본 데이터타입(물건) -> Wrapper 클래스 타입 객체(박스)로 자동(Auto) 변환(포장)
		 * 	 => Heap 공간의 객체 박스 (Wrapper 클래스 타입)에 기본 데이터타입 변수(물건)을
		 * 		전달하여 객체 형태로 포장하는 기능
		 * 	 => 컴파일러에 의해 자동으로 변환(포장)되어 진다.
		 * 
		 * 2. 오토 언박싱 (Auto Unboxing)
		 * - Wrapper 클래스타입 객체(박스)의 포장을 뜯어서 기본데이터타입(물건) 꺼내기
		 * 	 => Heap 공간의 객체 박스(Wrapper 클래스 타입)에 들어있는 물건(기본 데이터타입 값)을
		 * 		꺼내서 기본 데이터타입으로 변환
		 * 	 => 컴파일러에 의해 자동으로 변환(포장 뜯기) 되어 진다.
		 * 
		 * 
		 * */
...
//		System.out.println(n4);
		...
		//		=> Wrapper 클래스 타입 변수 = Wrapper클래스명.valueOf(기본형 데이터 또는 변수);
		//			=> 기본데이터타입을 Wrapper클래스 타입 객체로 변환하여 저장
//		n1 = Integer.valueOf(num1);
//		System.out.println("기본데이터타입 num1 = " + num1);
//		System.out.println("기본데이터타입 n1 = " + n1);
		
		
		
		// 2. Java JDK 1.5 이후 부터 사용 가능한 방법
		//		=>	Wrapper클래스타입 변수 = 기본데이터타입변수;
		//			=> 오토박싱 기능에 의해 기본데이터타입 -> 참조데이터타입으로 변환되어 저장
		n1 = num1;	// 오토박싱
		System.out.println("기본데이터타입 num1 = " + num1);
		System.out.println("기본데이터타입 n1 = " + n1);
		...
		num2 = n2.intValue();	// Integer타입 -> int타입
		System.out.println("기본데이터타입 num2 = " + num2);
		System.out.println("참조데이터타입 n2 = " + n2);	// toString 생략되어 있음, toString 오버라이딩
		
		// 2. Java JDK 1.5 이후 부터 사용 가능한 방법
		//		=>	기본데이터타입변수 = Wrapper클래스타입 변수;
		...
		Integer n5 = new Integer(40);
		int num5 = 40;
		System.out.println(n5 + num5);	// Integer -> int 오토언박싱
