...
public class Ex1 {
	...
	public static void main(String[] args) {
		 ...
		 * 	 클래스 정의 시 Object 클래스의 toString() 메서드를 오버라이딩을 통해
		 * 	 객체의 멤버변수 값들을 문자열로 결합하여 리턴하도록 해야 한다
		 * - 출력문 내에서 참조변수명.toString()을 호출하여 리턴값을 바로 출력하거나
		 * 	 리턴값을 String 타입 변수에 저장할 수 있다.
		 * 	 또한, 출력문 내에서는 toString()메서드 생략이 가능하므로
		 * 	 참조변수명을 출력문 내에서 전달 시 자동으로 toString() 메서드가 호출됨
		 * 	 ex) System.out.println(str); or System.out.println(str.toString());
		 * - 일반적으로 자바에서 제공되는 대부분의 API에는 toString() 메서드가
		 * 	 오버라이딩 되어 있으므로 객체내의 멤버변수값을
		 * 
		 * 
		 * */
		Person p1 = new Person("가나다", 20);
		System.out.println(p1.toString());	// Person@15db9742
		// toString() 메서드를 호출하면 "클래스명@해시코드값" 형태로 문자열 리턴됨
		...
		ToStringPerson p2 = new ToStringPerson("가나다", 20);
		System.out.println("객체 p2의 정보 : " + p2.toString());
		System.out.println("객체 p2의 정보 : " + p2);	// toString() 메서드 생략되어있음
